...
def main():
    ...
    try:
        ...
        subprocess.check_output(['/home/ag/.local/bin/stack', 'test', 'interpreter:test:graded-test'], stderr=subprocess.STDOUT, timeout=60)
    ...
    with tempfile.TemporaryFile() as tempf:
        proc = subprocess.Popen(['/home/ag/.local/bin/stack', 'test', 'interpreter:test:graded-test'], stdout=tempf)
    ...
    for line in tests_results_itmd:
        ...
        propPass = re.match(" *=P= (.*) \(([0-9]+) points\): *OK",line)
        propFail = re.match(" *=P= (.*) \(([0-9]+) points\): *FAIL",line)
        groupStart = re.match(" *=G= (.*)",line)
        wereDone = re.match("(.*tests passed.*)|(.*tests failed.*)",line)
