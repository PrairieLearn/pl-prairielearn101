class InstructionUtils {
    ...
    static registerAllBreakpoints() {
        InstructionUtils.instructionList
            .filter(e => e.isBreakpoint)
            .forEach(e => {
                Module.addBreakpoint(e.address);
            });
    }
    ...
    static highlightScrollIntoView(override = false) {
        const shouldScroll = Elements.textScrollToggle.checked;
        if (override || shouldScroll) {
            InstructionUtils.highlighted.scrollIntoView({block: "nearest"});
        }
    }

    static formatCode() {
        PLMipsDebugger.highlightWorker
            .postMessage(InstructionUtils.instructionList
                .map(e => e.instructionElement.innerHTML));
    }
